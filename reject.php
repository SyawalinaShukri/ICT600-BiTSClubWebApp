...
<div class="container">	
	...
	<div class="section-heading"><h2>MEMBER APPLICATION<em> REJECTION</em></h2></div>
	Member ID:
	...
	Approval:
	<input type="radio" name="mbr_approval" value="Rejected" <?php if ($row['mbr_approval']=='Rejected') echo "checked"?>> Rejected
	</br></br>
	Type:
	<input type="radio" name="mbr_type" value="Unqualified" <?php if ($row['mbr_type']=='Unqualified') echo "checked"?>> Unqualified
	</br>
	</br>
	Admin ID:
	<input type="text"  name="adminID" value="<?php echo $row['adminID']?>" ></br></br>
	<input type="submit" class="b" name="UpdateUser" value=" REJECT "></br>
	</form>
</section>
</div>
<?php
...
if (isset($_POST['UpdateUser'])) {
	...
	if ($_POST['member_id'] != '' && $_POST['studentID'] != '' && $_POST['adminID'] != '' && $_POST['mbr_name'] != '' && $_POST['mbr_gender'] != ''&& $_POST['mbr_prog'] != '' && $_POST['mbr_group'] != '' && $_POST['mbr_phone'] != '' && $_POST['mbr_email'] != '' && $_POST['mbr_pass'] != '' && $_POST['mbr_reg_date'] != ''&& $_POST['mbr_approval'] != ''&& $_POST['mbr_type'] != '') {
		...
		if ($result) {
            ...
            echo 'window.alert("Record rejected successfully");';
